...
function generateCSharp(brands) {
  ...
  for (const brand of brands) {
    const priorityOver = brand.priorityOver || [];
    const priorityOverStr = priorityOver.length > 0 
      ? `new[] { ${priorityOver.map(s => JSON.stringify(s)).join(', ')} }`
      : 'Array.Empty<string>()';
    
    lines.push('            new Brand');
    lines.push('            {');
    lines.push(`                Name = ${JSON.stringify(brand.name)},`);
    lines.push(`                PriorityOver = ${priorityOverStr},`);
